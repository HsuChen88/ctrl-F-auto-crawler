...
**自訂輸出路徑（便於分類）**：
```bash
uv run unified_collector.py --output outputs/<系所>
```
程式會自動建立不存在的目錄。

結構化留言／貼文（`comments_structural.jsonl`）為預設輸出，無須參數。若需額外擷取紀錄可加：

```bash
uv run unified_collector.py --raw --unknown
```

| 參數 | 說明 | 輸出檔案（位於 `--output` 目錄下） |
|------|------|--------|
| （預設） | 結構化留言／貼文（JSONL） | `comments_structural.jsonl` |
| `--raw` | 原始 GraphQL 攔截紀錄（JSONL） | `intercept_graphsql.jsonl` |
| `--unknown` | 未辨識的 GraphQL 請求（JSONL） | `unknown_graphql.jsonl` |
